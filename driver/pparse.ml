...
let apply_rewriters_str ?(restore = true) ~tool_name ast =
  List.fold_right (fun ppx acc ->
    match ppx with
    | External path -> rewrite Structure [path] acc
    | InProcess mapper ->
        Ast_mapper.(mapper.structure mapper acc)
  ) !installed_ppxs ast

let apply_rewriters_sig ?(restore = true) ~tool_name ast =
  List.fold_right (fun ppx acc ->
    match ppx with
    | External path -> rewrite Signature [path] acc
    | InProcess mapper ->
        Ast_mapper.(mapper.signature mapper acc)
  ) !installed_ppxs ast
...
let file ppf ~tool_name inputfile (type a) parse_fun (kind : a ast_kind) =
...
let parse_file ~tool_name kind ppf sourcefile =
  ...
  let ast =
    try file ppf ~tool_name inputfile (parse kind) kind
    with exn ->
